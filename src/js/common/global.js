...
langSwitch && (langSwitch.onclick = function() {
  let c_start = document.cookie.indexOf("cmlan=")
  if (c_start == -1) {
    s22t()
  } else {
    t22s()
...
function s22t() {
  ...
  walkText(document.body, 1)
}

function t22s() {
  walkText(document.body, 2)
  //以下删除cookie
  let date = new Date()
...
function walkText(node, type) {
  if (node.nodeType == 3) {
    node.data = type === 1 ? s2t(node.data) : t2s(node.data)
  }
  if (node.nodeType == 1 && node.nodeName != "SCRIPT") {
    for (var i = 0; i < node.childNodes.length; i++) {
      walkText(node.childNodes[i], type);
    }
  }
}
...
function myfun() {
  ...
  if (c_start == -1) {
    ...
    walkText(document.body, 1) //加载转化为繁体
  }
}
...
//功能：设置指定名称的 Cookie 值；
function writeCookie(name, value, days = null) {
  if (days != null) {
...
(function (globalDataStr) {
  ...
  document.querySelector('.icon-caidan').addEventListener('click', (e) => {
    e = window.event || e
    e.stopPropagation()
    if (!navShow) {
