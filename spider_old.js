...
//准备抓取的网站链接
,     dataUrl = 'http://www.quanxue.cn/CT_NanHuaiJin/ZhZiNHIndex.html'
...
http.get(dataUrl, function (res) {
  ...
  res.on('end', async function () {
    ...
    for (let i = 0, l = result.length; i < l; i++) {
      ...
      // arr_3.push(data[i])
      let id = '5d20c3609a091c7d27f1bdb8'
    ...
    http.get(href, function (res) {
      ...
      res.on('end', function () {
        ...
        $('.main p').each((k, v) => {
          ...
          if (k === 0) {
            let summary = $('.jingwen').text()
            let title = $('h2').text()
            res = '<p style="font-weight: bold;">' + summary + '</p><br>' + content + '<p style="font-weight:bold;font-size: 18px; text-align:center;line-height: 50px;">' + title + '</p>'
          } else {
            res = content
          }
          arr.push(res)
...
function getData(str) {
  ...
  arr.each(function(k, v) {
    ...
    if (/((\d+)章|前言)/.test(text)) {
      s = RegExp.$2
      dataTemp.push({
        title: text.replace(/\d+章/, s + '章 '),
        href: href
      })
      n = 1
    } else {
      if (n === 1) {
        dataTemp[index].title += ' ' + ('0' + n).slice(-2) + '节 ' + text
      } else {
        dataTemp.push({
          title: (s ? s + '章 ' : '前言 ') + ('0' + n).slice(-2) + '节 ' + text,
          href: href
        })
      }
      index++
